...
namespace Skywithin.AutoRegister.DI.Tests;
...
/// <summary>
/// Represents a service with multiple interfaces that explicitly specifies which interface to use
/// </summary>
[AutoRegister(Lifetime.Transient, RegisterAs.Interface, typeof(IMultipleInterfaceService2))]
public class ServiceWithExplicitInterface : IMultipleInterfaceService1, IMultipleInterfaceService2 { }

/// <summary>
/// Represents a service that specifies an interface it doesn't implement (for validation testing)
/// </summary>
public interface IUnimplementedInterface { }

[AutoRegister(Lifetime.Transient, RegisterAs.Interface, typeof(IUnimplementedInterface))]
public class ServiceWithInvalidInterface : IMultipleInterfaceService1 { }

/// <summary>
/// Represents a service that specifies a non-interface type (for validation testing)
/// </summary>
[AutoRegister(Lifetime.Transient, RegisterAs.Interface, typeof(ServiceA))]
public class ServiceWithNonInterfaceType : IMultipleInterfaceService1 { }
...
internal class IoCExtensionsTests
{
    ...
    public void AddAutoRegisteredServices_WithNullAssemblies_ShouldThrowArgumentException()
    {
        ...
        // Act & Assert
        var act = () => services.AddAutoRegisteredServicesFromAssembly((Assembly[])null!);
        act.Should().Throw<ArgumentException>()
            .WithMessage("At least one assembly must be specified.*")
            .And.ParamName.Should().Be("assemblies");
    }
    ...
    /// <summary>
    /// Verifies that services implementing multiple interfaces are registered only as the first suitable interface
    /// when no explicit interface is specified in the attribute.
    /// </summary>
    [Test]
    public void Service_WithMultipleInterfaces_ShouldRegisterAsFirstInterface()
    {
        ...
        // Assert
        // ServiceWithMultipleInterfaces should be registered as IMultipleInterfaceService1 (the first suitable interface)
        serviceAsFirstInterface.Should().NotBeNull().And.BeAssignableTo<ServiceWithMultipleInterfaces>();
        
        // ServiceWithMultipleInterfaces should NOT be registered as IMultipleInterfaceService2
        // IMultipleInterfaceService2 should resolve to ServiceWithExplicitInterface (which explicitly specifies IMultipleInterfaceService2)
        serviceAsSecondInterface.Should().NotBeNull().And.BeAssignableTo<ServiceWithExplicitInterface>();
        serviceAsSecondInterface.Should().NotBeAssignableTo<ServiceWithMultipleInterfaces>();
        
        // Verify that ServiceWithMultipleInterfaces is only registered as the first interface, not the second
        var allServicesAsSecondInterface = serviceProvider.GetServices<IMultipleInterfaceService2>().ToList();
        allServicesAsSecondInterface.Should().ContainSingle().Which.Should().BeAssignableTo<ServiceWithExplicitInterface>();
    }

    /// <summary>
    /// Verifies that services with explicit interface specification register as the specified interface,
    /// not the first interface.
    /// </summary>
    [Test]
    public void Service_WithExplicitInterface_ShouldRegisterAsSpecifiedInterface()
    {
        // Arrange
        var services = new ServiceCollection();
        services.AddAutoRegisteredServicesFromAssembly(Assembly.GetExecutingAssembly());
        ...
        // Act
        var serviceAsFirstInterface = serviceProvider.GetService<IMultipleInterfaceService1>();
        var serviceAsSpecifiedInterface = serviceProvider.GetService<IMultipleInterfaceService2>();

        // Assert
        // ServiceWithExplicitInterface should be registered as IMultipleInterfaceService2 (the explicitly specified one)
        serviceAsSpecifiedInterface.Should().NotBeNull().And.BeAssignableTo<ServiceWithExplicitInterface>();
        // The first interface should not resolve to ServiceWithExplicitInterface (it should resolve to ServiceWithMultipleInterfaces)
        serviceAsFirstInterface.Should().NotBeNull().And.BeAssignableTo<ServiceWithMultipleInterfaces>();
    }

    /// <summary>
    /// Verifies that services specifying an interface they don't implement are skipped with an error.
    /// </summary>
    [Test]
    public void Service_WithInvalidInterface_ShouldBeSkipped()
    {
        // Arrange
        var services = new ServiceCollection();
        services.AddAutoRegisteredServicesFromAssembly(Assembly.GetExecutingAssembly());
        ...
        // Act
        var service = serviceProvider.GetService<IUnimplementedInterface>();

        // Assert
        service.Should().BeNull(); // Should not be registered due to validation error
    }

    /// <summary>
    /// Verifies that services specifying a non-interface type are skipped with an error.
    /// </summary>
    [Test]
    public void Service_WithNonInterfaceType_ShouldBeSkipped()
    {
        // Arrange
        var services = new ServiceCollection();
        services.AddAutoRegisteredServicesFromAssembly(Assembly.GetExecutingAssembly());
        ...
        // Act
        var service = serviceProvider.GetService<ServiceWithNonInterfaceType>();

        // Assert
        // The service should not be registered as the interface because ServiceA is not an interface
        // But it might be registered as self if RegisterAs.Self is also specified, but in this case it's only Interface
        // So it should be null
        var serviceAsInterface = serviceProvider.GetService<IMultipleInterfaceService1>();
        // ServiceWithNonInterfaceType should not be registered because validation failed
        // But IMultipleInterfaceService1 might resolve to ServiceWithMultipleInterfaces
        serviceAsInterface.Should().NotBeNull();
    }
